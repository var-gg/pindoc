...
| `pindoc.harness.install` | ✅ implemented | `pindoc init` CLI 없이 MCP 호출만으로 PINDOC.md body 반환. 파일 쓰기는 에이전트 책임. 출력에는 workspace detection용 YAML frontmatter(project_slug/project_id/locale/schema_version), Applicable Rules Section X, chip-driven Task lifecycle Section 12가 포함된다. |
