...
| `pindoc.project.current` | ✅ implemented | **스펙의 `project.switch/list`는 미구현.** 현재 한 서버 프로세스 = 한 프로젝트 (env `PINDOC_PROJECT`). Phase 9부터 응답에 `capabilities` 블록 포함. Phase 14a에서 `scope_mode: "fixed_session"`, `new_project_requires_reconnect: true`, `receipt_ttl_sec: 1800`, `requires_expected_version: true`, `public_base_url` (DB의 server_settings) 추가. `auth_mode`는 `"trusted_local"` (Phase 14a rename from `"none"`). |
| `pindoc.project.create` | ✅ implemented | Phase 8 신규. 프로젝트 row 삽입 + `misc`/`_unsorted` area seed + 4 template artifact seed (Phase 13). Phase 14b부터 응답에 `reconnect_required: true`, `activation: "not_in_this_session"`, `next_steps[]` — "create됐지만 active session은 old project에 묶임"을 machine-readable로 반환. |
