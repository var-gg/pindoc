...
| `pindoc.asset.upload` | ✅ implemented | Project-scoped Asset 생성. `bytes_base64`/`content_base64`와 loopback-only `local_path`를 지원한다. `local_path`는 MCP 서버 host/container에서 평가되므로 Docker Desktop on Windows의 `A:\...` host path는 직접 읽을 수 없고, 실패 시 `ASSET_LOCAL_READ_FAILED`와 `tools/push-asset.ps1` 또는 base64 upload suggested action을 반환한다. |
| `pindoc.asset.read` | ✅ implemented | Asset UUID, sha256, `pindoc-asset://...` ref로 metadata와 Reader blob URL을 읽는다. storage key/local path는 노출하지 않는다. |
| `pindoc.asset.attach` | ✅ implemented | Asset을 artifact head revision에 `inline_image`/`attachment`/`evidence`/`generated_output`으로 연결한다. body Markdown 삽입은 별도이며 attach는 revision metadata다. |
...
| 7a | `pindoc.asset.upload` | writer |
| 7b | `pindoc.asset.read` | reader+ |
| 7c | `pindoc.asset.attach` | writer |
...
Asset MCP 도구는 artifact body와 분리된 immutable blob lifecycle을 다룬다.
`pindoc.asset.upload`는 새 Asset을 만들고, `pindoc.asset.read`는 metadata와
Reader blob URL을 반환하며, `pindoc.asset.attach`는 artifact의 현재 head
revision에 Asset relation을 붙인다.
...
{
  project_slug?: string,
  local_path?: string,       // MCP server host/container에서 읽는 경로
  bytes_base64?: string,     // client가 읽은 file bytes
  content_base64?: string,   // bytes_base64 alias
  filename?: string,
  mime_type?: string
}
```

`local_path`는 loopback caller 전용이며, Pindoc이 Docker Desktop의 Linux
container 안에서 실행 중이면 Windows host의 `A:\...` 경로는 보이지 않는다.
이 경우 서버가 host filesystem을 직접 읽으려 하지 않고
`ASSET_LOCAL_READ_FAILED` not_ready를 반환한다.

권장 우회 경로:

- MCP client가 파일을 읽을 수 있으면 `bytes_base64` 또는 `content_base64`와 `filename`/`mime_type`을 보낸다.
- Docker Desktop on Windows에서는 `tools/push-asset.ps1 -Path <Windows path> -ProjectSlug <project>`로 파일을 `/tmp/pindoc-asset-upload/...`에 복사한 뒤, 스크립트가 출력한 `local_path`를 `pindoc.asset.upload`에 전달한다.
- Reader-visible inline image는 upload 후 body Markdown에 `![alt](asset.blob_url)`를 넣고, 같은 Asset을 `pindoc.asset.attach(role="inline_image")`로 연결한다.

### Not ready codes

| Code | 의미 |
|---|---|
| `ASSET_INPUT_REQUIRED` | `local_path`와 base64 input이 모두 비어 있음 |
| `ASSET_INPUT_CONFLICT` | `local_path`와 base64 input을 동시에 보냄 |
| `ASSET_LOCAL_PATH_LOOPBACK_ONLY` | non-loopback caller가 `local_path`를 사용함 |
| `ASSET_LOCAL_READ_FAILED` | MCP 서버 host/container에서 `local_path`를 읽을 수 없음. Windows host path라면 `tools/push-asset.ps1` 또는 base64 upload로 전환 |
| `ASSET_BYTES_BASE64_INVALID` | base64 input decode 실패 |
| `ASSET_MIME_UNSUPPORTED` | allowlist 밖 MIME type |
