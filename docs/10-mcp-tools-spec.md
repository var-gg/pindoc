...
| `pindoc.project.current` | ✅ implemented | account-level + per-call project scope. stdio와 streamable_http 모두 `scope_mode: "per_call"`, `new_project_requires_reconnect: false`; HTTP 데몬 URL은 단일 `/mcp`이고 각 tool input의 `project_slug`가 scope를 결정한다. `auth_mode`는 `PINDOC_AUTH_MODE` 값을 반영한다(default `"trusted_local"`). Phase 9부터 `capabilities` 블록, Phase 14a에서 `receipt_ttl_sec: 1800`, `requires_expected_version: true`, `public_base_url`(server_settings) 추가, `auth_mode`는 Phase 14a rename from `"none"`. `receipt_exemption_limit`은 receipt-less bootstrap create 허용 한도(default 5)를 노출한다. |
...
- **인증**: 현재 V1 local transport는 stdio와 streamable_http 모두 기본
  `auth_mode="trusted_local"`이다. `PINDOC_AUTH_MODE` enum은
  `trusted_local|public_readonly|single_user|oauth_github`를 받지만, V1에서
  실제 resolver chain을 구성하는 값은 `trusted_local`뿐이다. 나머지는 startup
  error로 거절한다. MCP config에 `Authorization` header를 넣지 않는다.
  자기-호스팅 공개 / SaaS 시점에 GitHub OAuth + agent token을 별도 Decision으로
  도입한다.
...
현재 V1 local mode는 loopback `trusted_local` 신뢰 모델이다.
`PINDOC_AUTH_MODE` 기본값은 `trusted_local`이고, `public_readonly`,
`single_user`, `oauth_github`는 예약 enum으로만 존재한다. 구현 전 값으로
기동하면 서버가 fail-fast한다. HTTP MCP config는 단일 account-level URL만
둔다.
