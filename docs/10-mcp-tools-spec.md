...
가 `en`이 아닌데 `title`이 ASCII 영어 단어만으로 구성된 경우 create/update
모두에서 발생한다. 발행은 막지 않는다. 한글/한자/히라가나/가타카나가 한
글자라도 포함되면 이 warning은 발생하지 않으며, 응답의 `suggested_actions[]`
에는 title 보정용 `pindoc.artifact.propose(update_of=..., title=...)` 흐름과
body wording cleanup용 `pindoc.artifact.wording_fix` 힌트가 포함된다.
