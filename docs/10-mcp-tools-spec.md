...
| `pindoc.artifact.propose` | 🟡 partial | `update_of` + `commit_msg`로 revision 경로 열림 (Phase 7). **스펙의 `intent.kind=new\|modification\|split\|supersede`, `target_id`, `source_ids`, `body: TypedBody`, `pins[]`, `related_resources[]`는 미구현**. Phase 11에서 `body_json` minimal + semantic conflict + `pins[]` + `expected_version` + `supersede_of` + **`basis.search_receipt` (search/context 호출 시 서버 발급 opaque token, TTL 10분, write gating에 사용)** + `relates_to[]` 도입 예정. 공통 envelope은 `ok\|error\|not_ready`가 아니라 `accepted\|not_ready` — Phase 12에서 machine-readable `not_ready` 리포맷 (stable code: `NO_SRCH`, `NEED_PIN`, `NEED_VER`, `AREA_BAD`, `POSSIBLE_DUP`, `DBG_NO_REPRO`, `DEC_NO_ALT`) 예정. |
| `pindoc.artifact.search` | ✅ implemented | **Default embedder는 `stub` (hash 기반)** — Phase 10에서 real embedder로 전환 예정. 응답 notice에 `stub` 경고. Phase 9부터 hit마다 `agent_ref` + `human_url`. Phase 11부터 응답에 **`search_receipt`** (opaque token, TTL 10분) 포함 — 같은 세션 내 이후 propose 호출에서 `basis.search_receipt`로 제시해야 write gate 통과. |
