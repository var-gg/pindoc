...
| 2026-04-22 | **제목·H2 규범 + pre-flight warning** | rune count 기반 title 길이 체크(15-80), body H1 금지(title이 이미 H1), Type별 필수 H2 slot fuzzy match(en/ko 동의어). Warning-level 일관(block 아님 — retry loop 회피). Title shape 정규식 강제 안 함(언어 편향 회피). 기존 19 artifact retro-check 안 함, 신규 propose부터 적용. Smoke test: `_test_preflight_smoke` 임시 artifact로 `BODY_HAS_H1_REDUNDANT` + `MISSING_H2:*` 3건 발동 확인 후 archive. | `internal/pindoc/mcp/tools/artifact_propose.go` (`titleLengthWarnings`·`bodyH1Warnings`·`requiredH2Warnings`·`requiredH2ByType`·`updatePathWarnings`) |
