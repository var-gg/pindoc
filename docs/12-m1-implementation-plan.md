...
## Phase 17 — Bundled EmbeddingGemma 복원 (완료 · 2026-04-22)

Phase 10에서 저자 dev 환경 편의로 `http` + Docker TEI sidecar로 임시 전환했던 embedder를 **원래 Decision(2026-04-21, decisions.md §EmbeddingGemma default)에 복원**. 배포 타겟(EC2 medium / Hetzner CX22, ~$5/월)에서 Postgres + Pindoc + 임베딩까지 한 인스턴스에 들어가는 ~200MB footprint가 필수. TEI는 해당 tier에서 RAM 압박 가능.

**구현**:
- 의존: [`yalue/onnxruntime_go` v1.27](https://github.com/yalue/onnxruntime_go) + [`eliben/go-sentencepiece`](https://github.com/eliben/go-sentencepiece) + [`onnx-community/embeddinggemma-300m-ONNX`](https://huggingface.co/onnx-community/embeddinggemma-300m-ONNX)
- 신규 파일: `internal/pindoc/embed/runtime.go` (onnxruntime shared lib auto-download, 5 플랫폼 — Windows amd64 / Linux amd64·arm64 / macOS amd64·arm64), `gemma_model.go` (모델 + tokenizer assets auto-download from HuggingFace Hub), `gemma.go` (Provider: tokenize → ONNX session → mean-pool with attention mask → L2 normalize)
- `registry.go`: default가 gemma로 전환. **Silent stub fallback 제거** — `PINDOC_EMBED_PROVIDER` 미지정 = gemma, `=stub`은 명시 opt-in (unit test 용도), 알 수 없는 provider name은 **기동 실패**.
- Cache 경로: `<user cache>/pindoc/runtime/onnxruntime-1.24.1/` + `<user cache>/pindoc/models/embeddinggemma-300m/`. 첫 run 합 ~500MB download(빌드된 바이너리 + ONNX 모델), 이후 cache hit → 11.9s 초기 로드 → 배치 embed 0.1s.
- Variant override: `PINDOC_EMBED_GEMMA_VARIANT=q4|q4f16|quantized|fp16` (기본 q4 = 197MB 가중치).
- Dimension은 768 유지 → pgvector column 변경 불요. `pindoc-reembed` CLI로 기존 TEI 벡터 → gemma 벡터 in-place 교체 (migration 없음).
- Smoke test (`cmd/pindoc-embed-smoke`): "how do I retry a failed payment?" 쿼리 → retry 문서 0.5861 / 바나나 빵 문서 0.0359 → semantic ranking 정확.

**Windows dev 환경**: cgo 필수. MinGW-w64 gcc 15.2 (WinLibs) `C:\Users\{user}\tools\mingw64\bin\` 설치 권장. Linux/macOS는 기본 toolchain OK.

**OSS 사용자 대응**: `http` provider는 그대로 유지 — TEI·Ollama·OpenAI·Cohere·Vertex 어댑터로. docs/03 §Embedding Layer의 4분류 표(Bundled / Sidecar-local / Cloud / Stub-test)가 swap guide.
