...
var bundle = Bundle{
	"en": {
		...
		"preflight.task_status_via_transition_tool":   "✗ task_meta.status cannot be changed through shape=meta_patch. Status transitions still live on a dedicated lane so the acceptance-checklist and Implementer ≠ Verifier gates keep applying. Interim guidance: pindoc.task.transition is not available yet — use update_of + shape=body_patch with task_meta.status=<new> for open/blocked/cancelled, keep acceptance checkboxes 100% checked for claimed_done, and call pindoc.artifact.verify for verified. Omit task_meta.status from this meta_patch call and send assignee / priority / due_at only.",
	...
	"ko": {
		...
		"preflight.task_status_via_transition_tool":   "✗ task_meta.status는 shape=meta_patch로 변경할 수 없음. status 전이는 acceptance checklist 게이트와 Implementer ≠ Verifier 원칙을 지키기 위해 별도 경로로만 허용된다. 임시 가이드: pindoc.task.transition은 아직 구현되지 않았으므로 open/blocked/cancelled는 update_of + shape=body_patch + task_meta.status=<new>로 전이하고, claimed_done은 같은 경로에서 acceptance checkbox 100% 체크를 유지하며, verified는 pindoc.artifact.verify를 호출해야 한다. 이번 meta_patch 호출에서는 task_meta.status를 빼고 assignee / priority / due_at만 전송.",
