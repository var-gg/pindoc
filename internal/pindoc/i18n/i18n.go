...
var bundle = Bundle{
	"en": {
		...
		"preflight.task_status_via_transition_tool": "✗ task_meta.status cannot be changed through shape=meta_patch. Status transitions go through pindoc.task.transition (for open/blocked/cancelled) or pindoc.artifact.verify (for verified) so the acceptance-checklist and Implementer ≠ Verifier gates still apply. Omit task_meta.status from this call and send assignee / priority / due_at only.",
	...
	"ko": {
		...
		"preflight.task_status_via_transition_tool": "✗ task_meta.status는 shape=meta_patch로 변경할 수 없음. status 전이는 pindoc.task.transition (open/blocked/cancelled) 또는 pindoc.artifact.verify (verified) 경로 — acceptance checklist 게이트와 Implementer ≠ Verifier 원칙이 그 경로에서만 강제됨. 이번 호출에서는 task_meta.status를 빼고 assignee / priority / due_at만 전송.",
