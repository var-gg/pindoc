...
type artifactProposeInput struct {
	...
	BodyMarkdown  string   `json:"body_markdown,omitempty" jsonschema:"main content in markdown; omit on update_of when body_patch is supplied. To render an uploaded image inline, include Markdown image syntax using the returned asset.blob_url; asset.attach stores metadata but does not edit the body."`
	...
	Slug          string   `json:"slug,omitempty" jsonschema:"optional; auto-generated from title if absent. Recommended when the title is long or mixed-language: pass a concise stable slug up front so follow-up update_of/asset.attach calls do not have to quote a verbose auto slug."`
	Tags          []string `json:"tags,omitempty"`
	Completeness  string   `json:"completeness,omitempty" jsonschema:"draft|partial|settled; default partial. draft is a visible maturity/trust state after accepted propose, not a hidden private draft; use visibility/private or review workflow for access control."`
	...
	BodyPatch *BodyPatchInput `json:"body_patch,omitempty" jsonschema:"lightweight patch against the current body — update_of only, exclusive with body_markdown. Examples: {mode:'append',append_text:'...'}, {mode:'section_replace',section_heading:'Outcome',replacement:'...'}, {mode:'checkbox_toggle',checkbox_index:0,checkbox_state:true}"`
...
type TaskMetaInput struct {
	...
	ParentSlug               string `json:"parent_slug,omitempty" jsonschema:"slug of parent Task artifact for board hierarchy only; use relates_to for semantic implements/blocks/evidence relationships"`
...
type ArtifactMetaInput struct {
	SourceType               string   `json:"source_type,omitempty" jsonschema:"code | artifact | user_chat | external | mixed; trust substrate surfaced in artifact.read/context.for_task and used for safe defaults"`
	ConsentState             string   `json:"consent_state,omitempty" jsonschema:"not_needed | requested | granted | denied; consent classification for user_chat/mixed sources, warning/privacy guidance only"`
	Confidence               string   `json:"confidence,omitempty" jsonschema:"low | medium | high; trust signal surfaced in artifact.read and context.for_task landings"`
	Audience                 string   `json:"audience,omitempty" jsonschema:"owner_only | approvers | project_readers; privacy/trust hint, use visibility for access control"`
	NextContextPolicy        string   `json:"next_context_policy,omitempty" jsonschema:"default | opt_in | excluded; controls future retrieval: default surfaces normally, opt_in when specifically asked, excluded hidden from context surfaces"`
	VerificationState        string   `json:"verification_state,omitempty" jsonschema:"verified | partially_verified | unverified; evidence status surfaced in artifact.read/trust summaries"`
...
const artifactProposeToolDescription = "Propose a new artifact (the only write path humans use — always via an agent). Create path (both update_of and supersede_of omitted) requires basis.search_receipt from pindoc.artifact.search or pindoc.context.for_task in the same session; update/supersede paths do not. Accepted propose auto-publishes the artifact; completeness=draft is visible in the Reader when visibility permits and means immature/needs review, not hidden/private. If the title is long, pass a short explicit slug up front; later update_of and asset.attach calls must quote the accepted response slug exactly. Update path may send body_patch instead of body_markdown: use update_of + expected_version + body_patch, omit body_markdown, and set shape=body_patch or omit shape. body_patch examples: append uses {mode:'append', append_text:'...'}; section_replace uses {mode:'section_replace', section_heading:'Outcome', replacement:'...'}; checkbox_toggle uses {mode:'checkbox_toggle', checkbox_index:0, checkbox_state:true}. dry_run=true validates without INSERT/update and does not bypass receipt, expected_version, relates_to, or policy gates; use it for agent self-validation and MCP regression tests, then retry with dry_run=false to publish. Uploaded images render inline only when body_markdown includes Markdown image syntax with asset.blob_url; asset.attach records revision metadata. For shape=acceptance_transition, checkbox_label_match is a fuzzy unresolved-label selector; use checkbox_index when exactness matters. For Task creates, omitted task_meta.assignee means unassigned; explicit assignee claims ownership, and explicit task_meta.assignee=\"\" clears on update. Task priority meanings: p0 release blocker, p1 must close before release, p2 next round, p3 backlog; project-specific priority policy wins when present. task_meta.parent_slug is board hierarchy only; use relates_to for semantic relationships such as implements, blocks, references, and evidence. artifact_meta.source_type/confidence/verification_state surface trust in read/context views; artifact_meta.next_context_policy affects future retrieval; artifact_meta.audience/consent_state frame privacy and consent but do not replace visibility. Use pins[] for concrete code/file/URL evidence; use relates_to relation=evidence when the supporting source is another Pindoc artifact. Returns Status=accepted + artifact_id on success, or Status=not_ready + checklist + suggested_actions if Pre-flight fails. Always read the checklist; never surface the raw error to the user without trying the suggested actions first."
...
type BodyPatchInput struct {
	Mode           string `json:"mode" jsonschema:"section_replace | checkbox_toggle | append. Examples: {mode:'append',append_text:'...'}, {mode:'section_replace',section_heading:'Outcome',replacement:'...'}, {mode:'checkbox_toggle',checkbox_index:0,checkbox_state:true}"`
